...
export const S12Modal = ({ onClose }: { onClose: () => void }) => {
  ...
  const handleCapture = async () => {
    ...
    try {
      const canvas = await html2canvas(document.body, {
        ...
        logging: false,
        // Garante que o renderer ignore o que não deve sair na foto
        ...
        onclone: (clonedDoc) => {
          ...
          for (let i = 0; i < elements.length; i++) {
            ...
            // LIMPEZA AGRESSIVA: Se houver oklch em qualquer propriedade, resetamos.
            // Isso evita que o parser do html2canvas quebre ao ler o estilo computado.
            ...
            // Força visibilidade do mapa se necessário
      ...
      const fileName = `S12_${locality.replace(/\s+/g, "_")}_${featureName}.jpg`;
      ...
      // Se ainda falhar, o culpado é o CSS global do Tailwind 4
      alert(
  ...
  return (
    <div className="fixed inset-0 z-[10000] flex items-center justify-center pointer-events-none no-print overflow-hidden bg-transparent">
      {/* BOTÕES - Marcados com no-print para não saírem na foto */}
      ...
      <div className="relative flex flex-col items-center pointer-events-none w-[95vw] max-w-[1600px]">
        ...
        >
          {/* Label do visor - no-print para não sujar o mapa */}
        ...
        {/* Textos Informativos (Fora do Retângulo Amarelo) */}
